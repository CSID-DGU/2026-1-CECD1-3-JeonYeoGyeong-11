...
class GateCheckEntrypointTest(unittest.TestCase):
    ...
    def test_current_gate5b_prep_contract_passes(self):
        module = load_run_module()

        report = module.run_gate_check("5b-prep", ROOT)

        self.assertTrue(report["pass"], report["failed_checks"])
        self.assertEqual(report["gate"], "5b-prep")

    def test_current_gate5c_prep_contract_passes(self):
        module = load_run_module()

        report = module.run_gate_check("5c-prep", ROOT)

        self.assertTrue(report["pass"], report["failed_checks"])
        self.assertEqual(report["gate"], "5c-prep")

    def test_current_gate5d_prep_contract_passes(self):
        module = load_run_module()

        report = module.run_gate_check("5d-prep", ROOT)

        self.assertTrue(report["pass"], report["failed_checks"])
        self.assertEqual(report["gate"], "5d-prep")
