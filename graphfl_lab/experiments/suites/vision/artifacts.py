...
RESULT_JSON_GLOBS = (
    ("result_general_*.json", "result_general_"),
    ("result_vision_*.json", "result_vision_"),
)
...
def discover_result_json_paths(suite_dir: Path) -> dict[str, Path]:
    """Map result JSON suffix keys to paths, preferring canonical vision aliases."""
...
def load_suite_rows_json(out_dir: Path) -> list[dict[str, Any]]:
    """Load suite row metadata from canonical or compatibility JSON artifacts."""
