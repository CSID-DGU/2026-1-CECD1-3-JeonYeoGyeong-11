...
class GraphFLDiagnosticStrategy(_EvalTracer, fl.server.strategy.FedAvg):
    ...
    def _diagnostic_post_flat_updates(
        ...
        *,
        ...
        target = normalize_key(target_override or self.aggregation_target)
        global_flat = flatten_weights(self._current_global).astype(
        ...
        if target in {"update", "delta", "update_delta"}:
            mat = np.stack(
                [
                    flatten_weights(arr).astype(np.float64, copy=False)
                    for arr in local_updates
                ],
                axis=0,
            )
