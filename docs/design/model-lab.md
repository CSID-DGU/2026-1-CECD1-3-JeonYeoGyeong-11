...
| federated_protected | 데이터는 seller별, C에는 보호된 제출 | 보호 모듈 G4 통과 후 R1/R2. A 화면 완성은 불필요 |
...
공개 데이터라는 이유로 기존 synthetic_plaintext 경로에 실데이터를 연결하지 않는다. 여러 판매자 원자료를 합쳐 한 optimizer로 학습한 결과는 FL 결과로 보고하지 않는다.
...
- 구매마다 전체 모델을 학습하지 않는다. 거래·특징 반영, 추천, 개인화, FL 라운드는 별도 작업이다. 구체적인 트리거·빈도는 A08에서 확정한다.
