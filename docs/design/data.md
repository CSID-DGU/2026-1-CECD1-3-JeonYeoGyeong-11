...
원자료, 고객별 배정표, 전처리 구매 이벤트, 관계·캐시·개별 업데이트·실데이터 모델은 기본 Git 제외다. 공개할 수 있는 schema·직접 만든 합성 fixture·집계 보고서만 별도 검토한다. 라이선스 확인과 공개 범위 기록은 B가 맡는다. 이런 파일을 처음 올리는 PR의 검토는 [Git 협업](../team/git-workflow.md) 첫 표를 따른다.
