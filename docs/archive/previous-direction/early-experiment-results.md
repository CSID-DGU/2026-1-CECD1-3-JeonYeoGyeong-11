...
## Post-Fix Graph-Filter-Only Sweep
...
```text
experiments_current/lowpass_stress_graph_filter_only_n20_n50_postfix/client_count_sweep_summary.md
```
...
fedavg
fedavgm
ours_head_graph_knn_k1_lp0p5_fixed_tau_graph_filter_only
ours_head_weight_graph_filtered_weight_agg_knn_k1_lp0p5_fixed_tau_graph_filter_only
ours_ema_graph_knn_k1_lp0p5_fixed_tau_graph_filter_only
ours_ema_signal_knn_k1_lp0p5_fixed_tau_graph_filter_only
...
```powershell
python run_vision_client_count_sweep.py --config configs/vision/smoke/semantic_ema_weight.json
```
...
no crash from EMA or graph_filtered_weight path
```
