...
실제 연결 전에 정할 것은 [열린 구현 항목](design/open-questions.md), 검토·병합·인계는 [Git 협업](team/git-workflow.md)에만 적는다. 이 뼈대는 import·객체 수명·동기 호출·작업 실행 위치를 고정했고, 남은 결정을 임의 기본값으로 숨기지 않는다.
