...
| `experimental-design.md` added as core experiment guide | done |
...
| strategy runtime moved to `spectral_fl/strategies/graphfl/` | done |
| `spectral_fl/strategies/spectral/` compatibility wrappers | done |
...
| Name | Target | Risk |
|---|---|---|
| package `spectral_fl` | `graphfl_lab` | very high |
...
4. next: remaining spectral_filtered_* design/operator spellings
5. last: top-level package rename
```
...
```text
C0 confirm graphfl_lab
C1 add graphfl_lab/ alias package
C2 add Flower entrypoint bridge
C3 migrate internal imports in batches
C4 move implementation files
C5 update docs/commands
C6 run full checks
```
...
| P1 | package alias `graphfl_lab` | high |
| P1 | real package move to `graphfl_lab/` | very high |
