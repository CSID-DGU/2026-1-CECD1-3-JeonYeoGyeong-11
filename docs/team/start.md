...
역할은 사용자의 지정과 역할 카드 2행의 계정으로 확인한다. 둘 다 없거나 서로 충돌하면 사용자에게 확인하고, 임의로 다른 담당의 구현을 시작하지 않는다. 로그인한 GitHub 계정이 사용자 본인 것이고 카드 2행과 같은지도 확인받는다([작업 규칙](working-agreement.md) §1).
...
역할별 GitHub 계정은 회의 뒤 각 역할 카드 2행에 한 번에 기록한다(실명 없음, [작업 규칙](working-agreement.md) §1).
...
- 약 2주 완성이 목표지만 날짜별 진도나 전원 대기를 강제하지 않는다. 다만 [작업 규칙](working-agreement.md) §8의 세 점검 시점에는 결과를 PR에 올린다. 가능한 날에는 여러 단위를 진행하고 결과가 생기면 비동기로 피드백한다.
- 첫 완성의 범위, 점검 시점(D3·D7·D10), 줄이는 순서, 최종 산출물은 [작업 규칙](working-agreement.md) §8에만 적는다. 1차 대비는 [평가](../design/evaluation.md) §4 '사전에 정한 판정'이다. 전체 roster, 다수 seed, 고급 UI, 클라우드/자동 스케줄러는 후순위다.
...
- 서비스의 실데이터 유래 중앙 FL는 보호 경로 검증 전 실행하지 않는다. 공개 데이터 실험은 실험실 시뮬레이션으로 G4와 별도로 진행한다([결정](../design/decisions.md) D0020). 실험 규모를 줄여도 데이터 경계·시점 누출 방지·이벤트 정확성은 유지한다.
...
| D0017~D0021 | 채택한 설계 결정 | [결정 요약](../design/decisions.md) |
