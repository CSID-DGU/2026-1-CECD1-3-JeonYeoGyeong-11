...
초기 PR #3은 `main`에 병합되었고 CI·브랜치 보호·required check가 적용되며 CODEOWNERS가 리뷰를 요청한다. 역할별 계정은 각 역할 카드 2행에 기록한다. 세부 설정과 문서 규칙의 강제 범위는 Git 협업 문서를 따른다.
