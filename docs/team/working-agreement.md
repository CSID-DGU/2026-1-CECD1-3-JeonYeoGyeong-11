...
**프로세스 문서 동결.** AGENTS.md, [팀 시작 안내](start.md), 이 문서, [Git 협업](git-workflow.md), PR 템플릿, CODEOWNERS, CI와 `commerce/tools/`의 검사 규칙은 실제로 작업이 막혔거나 사고가 났을 때만 고친다. 헷갈리거나 모순으로 보이는 점은 제목에 `[process]`를 붙인 이슈로 모아 두고 §8의 D7 점검 때 한 번에 처리한다. 예외는 회의 뒤 역할 계정을 기입하는 PR이다. 역할 카드의 작업 내용과 설계 문서는 동결 대상이 아니다.
...
| 시점 | 확인할 결과 | 미달이면 |
| --- | --- | --- |
| D3 | C: HTTP와 분리한 집계 core(전원 완료 또는 폐기, 균등 평균)를 B가 호출할 수 있게 병합. 보호 방식 선정 기록과 작은 합성 예제(D0021 범위). B: Instacart 소형 표본의 어댑터와 text builder, torch를 넣은 lock PR(OQ18, C 검토). A: 주문 상태·transaction·outbox. 결정: OQ13·14·15(사람), OQ18(B·C 합의), OQ19(A·C 합의, 계약을 바꾸면 사람) | 집계 core가 없으면 B는 실행기 안에 같은 규칙의 임시 함수를 두고 진행한다. core가 병합되면 바꿔 끼우고 결과가 같은지 확인한다. 보호 방식이 보이지 않으면 C는 c1과 집계 core를 먼저 끝내고, G4는 조사 기록과 fail-closed로 마무리할 준비를 한다 |
| D7 | B: Instacart E-G0(text_only)와 기준선 수치, 텍스트 모델 크기와 계산 예산 확정. 관계 예산은 관계 특징의 첫 소형 실행에서 정한다. A: 평가 지표·기준선 모듈(B의 E-G0 채점에 쓴다), a1 일부와 이벤트 전달. C: c1 합성 라운드. 1차 대비 경로: text_only 실험실 FL 1라운드가 C 집계 core로 끝까지 돌고, 관계 특징의 첫 소형 실행이 끝났다. 결정: OQ16·17(사람) | 1차 대비 경로의 두 산출물 중 하나라도 없으면, 1차 대비를 D10 필수로 유지할지 팀이 정한다. 그 밖에는 아래 줄이는 순서대로 범위를 줄인다 |
...
사람 결정은 담당 역할이 선택지·추천안·영향을 이슈로 올리고 팀이 정한다. 담당 합의 항목은 해당 역할끼리 PR이나 이슈에서 정한다([열린 항목](../design/open-questions.md)의 '누가 정하나'). 기한까지 정하지 못하면 그 결정에 걸린 항목은 부분 완료로 두고 줄이는 순서를 적용한다.
