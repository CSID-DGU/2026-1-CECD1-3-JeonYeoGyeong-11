...
현재 직접 지원하는 기본 pipeline은 좁게 둔다. graph authoring은 모든 가능한
graph generator를 완성했다는 뜻이 아니라, 아래 기본 pipeline에서 출발해 각
구성요소를 교체하고 비교할 수 있는 구조라는 뜻이다.
...
확장 graph는 이 pipeline 중 어느 조각을 바꾸는지로 설명한다. 예를 들어 새로운
embedding 기반 방법은 client representation을 바꾸는 것이고, attention 기반
방법은 relation score 또는 edge weight를 바꾸는 것이며, learned topology는
...
가능한 한 모든 control은 같은 graph construction pipeline을 공유한다. 차이는
relation 의미, client identity 대응, fine-grained edge 구조, graph-free
correction처럼 검증하려는 요인에만 둔다. 이 비교 원칙이 있어야 real-control
gap을 graph-specific evidence로 더 안전하게 해석할 수 있다.
...
주요 지표는 같은 result schema 안에 함께 남기되, 해석 우선순위를 나눈다.
...
| secondary diagnostics | LOO, density/degree/entropy, smoothness/spectral energy | 비용이 크거나 보조적인 구조 해석을 제공 |
...
| `LOO` | client influence | 특정 client 제거가 graph 성능에 미치는 영향을 checkpoint diagnostic으로 본다 |
...
직접 연결되는 accuracy/loss, `real-control gap`, `graph-free control gap`,
`alignment`, `DI`, `N_eff`를 먼저 본다. `LOO`, topology metric, spectral
metric은 비용과 해석 범위를 고려해 보조 설명이나 checkpoint 분석으로 붙인다.
...
4. primary decision/mechanism metric과 secondary diagnostic을 남기는 result schema
5. 대표 vision dataset과 Cora 수준의 graph dataset에서 초기 검증이 가능한 prototype
