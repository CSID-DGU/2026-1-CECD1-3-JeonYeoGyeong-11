...
예제 해시는 형태 예시입니다. 실제 해시·배열 내용·인증·주문 복구·모델 학습·보호 집계는 각 구현 게이트에서 확인해야 합니다. 과거 확장용 허용값도 남아 있어 스키마 통과만으로 서비스 지원을 주장하지 않습니다.

로컬 Python 인터페이스는 `ports.py`, 반환 객체는 `types.py`, 오류는 `errors.py`에 있다. 호출 경로·소유권·다음 구현은 [개발 안내](../../../docs/development.md)를 따른다.
