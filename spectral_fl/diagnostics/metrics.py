...
def summarize_pre_post(
    *,
    flat_updates: np.ndarray,
    flat_updates_post: np.ndarray | None = None,
    weights_pre: Sequence[float],
    weights_post: Sequence[float],
    loo_enabled: bool,
) -> Dict[str, object]:
    g_pre = np.asarray(flat_updates, dtype=np.float64)
    g_post = (
        g_pre
        if flat_updates_post is None
        else np.asarray(flat_updates_post, dtype=np.float64)
    )
    if g_pre.shape != g_post.shape:
        raise ValueError(
            "flat_updates and flat_updates_post must have the same shape "
            f"(got {g_pre.shape} vs {g_post.shape})"
        )
    p_pre = _as_prob(weights_pre)
    p_post = _as_prob(weights_post)
    norms_pre = np.linalg.norm(g_pre, axis=1)
    norms_post = np.linalg.norm(g_post, axis=1)

    delta_pre = np.sum(p_pre[:, None] * g_pre, axis=0)
    delta_post = np.sum(p_post[:, None] * g_post, axis=0)

    q_pre = compute_q(norms_pre, p_pre)
    q_post = compute_q(norms_post, p_post)
    align_pre = compute_alignment(g_pre, delta_pre)
    align_post = compute_alignment(g_post, delta_post)
    loo_pre = (
        compute_loo_distortion(g_pre, p_pre, global_delta=delta_pre)
        if loo_enabled
        else np.zeros(g_pre.shape[0], dtype=np.float64)
    )
    loo_post = (
        compute_loo_distortion(g_post, p_post, global_delta=delta_post)
        if loo_enabled
        else np.zeros(g_post.shape[0], dtype=np.float64)
    )
