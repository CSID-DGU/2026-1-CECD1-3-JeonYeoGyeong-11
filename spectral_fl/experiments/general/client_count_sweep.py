"""Run matched General FL suites across different client counts.

This wrapper keeps all experiment knobs fixed except ``num_clients``.  Each
client count gets its own ``run_general_suite.py`` output directory, followed by
...
def run(args) -> None:
    ...
    for n_clients in args.client_counts:
        ...
        all_rows.extend(
            read_summary_rows(
                suite_dir / "general_suite_summary.csv",
