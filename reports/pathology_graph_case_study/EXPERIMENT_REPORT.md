...
```text
Compare graph correction against graph-free dominance correction and generic smoothing controls.
```
...
```text
Selected case: A
Rationale: graph methods beat controls with non-trivial delta perturbation and stay competitive.
```

Next step:

```text
stronger graph-source ablations
robustness checks before novelty claims
```
