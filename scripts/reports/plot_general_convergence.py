"""Plot round-wise convergence curves from General FL suite result JSONs.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Create convergence SVG/CSV plots from run_general_suite.py outputs."
        )
    )
    p.add_argument(
        "--suite-dir",
    ...
    p.add_argument(
        ...
        default=[],
        help=(
            "Optional individual result_general_*.json files. Multiple files "
    ...
    p.add_argument("--title", type=str, default="General FL convergence")
    p.add_argument(
        "--show-seeds",
        action="store_true",
...
def main() -> None:
    ...
    if not records:
        raise SystemExit("No matching result_general_*.json records found")
